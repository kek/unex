...
# Produce the dispatcher's .uc bundle — needs ucm, Elixir, and network access
# to Unison Share (to lib.install base/http/unex).
RUN mix unex.compile_dispatcher && test -s data/dispatcher.uc
...
# Copy release, healthcheck, and the pre-built dispatcher + runtime codebase.
COPY --from=build /app/_build/prod/rel/unex /app
COPY --from=build --chown=unex:unex /app/data /app/data
COPY healthcheck.sh /app/healthcheck.sh
RUN chmod +x /app/healthcheck.sh

# Ensure data dir exists with right ownership (COPY --chown takes care of
# pre-existing entries, but this covers empty-image case).
