defmodule Unex.API.AbilitiesAPITest do
  ...
  setup_all do
    ...
    case Process.whereis(Unex.Abilities.Scratch) do
      nil -> {:ok, _} = Unex.Abilities.Scratch.start_link([])
      _ -> :ok
    end

    case Process.whereis(Unex.Abilities.Log) do
      nil -> {:ok, _} = Unex.Abilities.Log.start_link([])
      _ -> :ok
    end
